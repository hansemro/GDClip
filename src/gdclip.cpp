...
bool GDClip::set_image_from_pbarray(PoolByteArray rgba8_image, uint32_t width, uint32_t height) {
    if (width && height && rgba8_image.size() >= width*height*4) {
        ...
        for (unsigned long pixel = 0; pixel < width*height; ++pixel) {
            data[pixel] =  (rgba8_image[pixel*4 + 0] & 0xff) << RED_SHIFT;  // R
            data[pixel] |= (rgba8_image[pixel*4 + 1] & 0xff) << GREEN_SHIFT;// G
            data[pixel] |= (rgba8_image[pixel*4 + 2] & 0xff) << BLUE_SHIFT; // B
            data[pixel] |= (rgba8_image[pixel*4 + 3] & 0xff) << ALPHA_SHIFT;// A
        }
